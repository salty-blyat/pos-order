                            ...
                            <nz-space [nzSplit]="spaceSplit">
                                ...
                                <ng-container *ngIf="isLookupEdit">
                                    <a *nzSpaceItem (click)=" uiService.showEdit(data.id || 0) " >
                                        <i  nz-icon nzType="edit" nzTheme="outline" class="edit" ></i>
                                        {{ "Edit" | translate }}
                                    </a>
                                </ng-container>
                                <ng-container *ngIf="isLookupRemove">
                                    <a *nzSpaceItem class="delete" (click)="uiService.showDelete(data.id || 0)">
...
export class LookupItemListComponent extends BaseListComponent<LookupItem> {
    ...
    override search() {
      if (this.lookupTypeId()) {
        ...
        setTimeout(() => {
          const filters: Filter[] = [{field: "search", operator: "contains", value: this.searchText()}];
          if (this.lookupTypeId()) {
            filters.push({field: "lookupTypeId", operator: "eq", value: this.lookupTypeId()});
          }
