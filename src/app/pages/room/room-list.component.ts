        ...
        <div nz-row >
          ...
          <div class="filter-box">
            <app-floor-select
              ...
              [showAllOption]="true"
              (valueChanged)="
                  floorId = $event; param.pageIndex = 1; search()
              "
            ></app-floor-select>
          </div>
          <div class="filter-box">
          ...
          <div class="filter-box">
              <nz-badge [nzDot]="hasAdvancedFilter">
                <button
                  nz-button
                  nzType="default"
                  (click)="uiService.showAdvancedFilter('room-list', this.advancedStoreKey)"
                >
          ...
          <tr *ngFor="let data of lists; let i = index">
            ...
            <td nzEllipsis>
              <ng-container *ngIf="isRoomTypeView">
                <a (click)="uiService.showView(data.id!)">{{ data.roomNumber }}</a>
              </ng-container>
              <ng-container *ngIf="!isRoomTypeView">
                {{ data.roomNumber }}
              </ng-container>
            ...
            <td nzEllipsis>
              <div class="show-tag">
                <nz-tag *ngFor="let tag of data.tagNames; let last = last">{{ tag }}{{ last ? '' : ' ' }}
                </nz-tag>
              </div>
            </td>
...
export class RoomListComponent extends BaseListComponent<Room> {
  ...
  override ngOnInit() {
    ...
    if (this.hasAdvancedFilter) {
      this.setAdvancedFilter(this.sessionStorageService.getValue(this.advancedStoreKey));
    }
    this.search();
  }
  ...
  override search() {
    ...
    setTimeout(() => {
      ...
      if (this.houseKeepingStatusId) {
        filters.push({ field: 'houseKeepingStatus', operator: 'eq', value: this.houseKeepingStatusId });
      }

      if (this.roomTypeId) {
        filters.push({ field: 'roomTypeId', operator: 'eq', value: this.roomTypeId });
      }

      if (this.roomStatusId) {
        filters.push({ field: 'status', operator: 'eq', value: this.roomStatusId });
      }

      if (this.tagIds.length > 0) {
        filters.push({ field: 'tagIds', operator: 'in', value: this.tagIds.join(',') });
      }
