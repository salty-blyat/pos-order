...
@Component({
  ...
  template: `
      <nz-select
              ...
              [nzDisabled]="disabled()"
      >
          <nz-option *ngIf="showAllOption()" [nzValue]="0" [nzLabel]="'AllMember' | translate"></nz-option>
          ...
          <ng-template #actionItem>
              <a *ngIf="addOption() && isMemberAdd()" (click)=" uiService.showAdd(componentId)" class="item-action">
...
export class MemberSelectComponent extends BaseSelectComponent<Member>{
  constructor(
    ...
    super(service, uiService, sessionStorageService, 'member-filter', 'all-member');
  }
