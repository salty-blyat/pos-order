...
@Component({
    ...
    >
      <nz-option *ngIf="showAllOption" [nzValue]="0" [nzLabel]="'AllTag' | translate"></nz-option>
      <nz-option nzCustomContent *ngFor="let item of lists"
                 [nzValue]="item.id"
                 [nzLabel]="item.name + ''"
      >
        <span style="color: rgba(0, 0, 0, 0.50);" class="b-name">{{ item.groupName }}</span>
...
export class TagMultiSelectComponent implements OnInit, ControlValueAccessor, OnDestroy{
  ...
  searchTags(){
    ...
    this.service.searchTags(this.param).subscribe((result:any) => {
      ...
      if(this.selectedValue?.filter(x => !ids.includes(x)).length > 0
        ...
        let idsFilter = JSON.stringify(this.selectedValue.filter(x => !ids.includes(x)));
        this.param.filters = JSON.stringify([{field: 'ids', operator: 'contains', value: idsFilter}]);
  ...
  setStorageKey(filter:any): void{
    if (this.storageKey) {
      ...
      this.sessionStorageService.setValue({key: this.parentStorageKey, value});
    }
  }
  ngOnDestroy(): void {
