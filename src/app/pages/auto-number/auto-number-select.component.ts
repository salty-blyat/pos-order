...
export class AutoNumberSelectComponent
  implements OnInit, ControlValueAccessor, OnDestroy
{
  constructor(
    private service: AutoNumberService,
  ...
  @Input() showAllLabel: string = 'all_auto_numbers';
  @Output() valueChanged = new EventEmitter<any>();
  parentStorageKey = 'auto-number-filter';
  componentId = uuidv4();
  disabled = false;
  loading = false;
  value: any = '';
  ...
  setStorageKey(filter: any): void {
    if (this.storageKey) {
      ...
      if (filter === 0) item = { id: 0, name: 'all_auto_numbers' };
