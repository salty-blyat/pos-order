      ...
      <form nz-form [nzLayout]="'vertical'">
        <nz-form-item>
          <nz-form-label *ngIf="formItem.label" style="padding: 0">
            <span style="font-size: 12px; font-weight: 700; color: rgba(0,0,0,.45)">{{formItem.label | translate}}</span>
          </nz-form-label>
...
export class ReportDynamicInputComponent implements AfterViewInit, ControlValueAccessor{
  constructor(private componentFactoryResolver: ComponentFactoryResolver,
              private changeDetectorRef: ChangeDetectorRef) {
  }

  @ViewChild('inputComponentRef', {read: ViewContainerRef}) inputComponentRef!: ViewContainerRef;
  ...
  ngAfterViewInit(): void {
    setTimeout(() =>{
      // @ts-ignore
      const componentFactory = this.componentFactoryResolver.resolveComponentFactory(ParamSelectComponent[this.formItem.type]);
      this.componentRef = this.inputComponentRef.createComponent<any>(componentFactory);
      ...
      try {
        let initParam = JSON.parse(this.formItem.initParam ?? "{}");
        this.componentRef.instance.applyInitParam(initParam);
      }
      catch (e){
      }
      try {
        this.componentRef.instance.applyDefaultValue(this.convertDefaultSelectValue());
      }catch (e){
        console.log(e)
      }
  ...
  private value:{id: string, label: string,value?: string} = {id: "0", label: ""};
